...
protected:
	// AdlEngine
	virtual Common::String loadMessage(uint idx) const override;
	virtual void printString(const Common::String &str) override;
	virtual char convertEvent(const Common::Event &event) const override;
	virtual Common::String inputString(const Common::String &prompt) override;
	virtual Common::String formatVerbError(const Common::String &verb) const override;
	virtual void getInput(uint &verb, uint &noun) override;
	virtual ScriptEnv *createScriptEnv(const Command &cmd, byte room, byte verb, byte noun) override;
	virtual void setupOpcodeTables() override;

	// AdlEngine_v2
	virtual DataBlockPtr readDataBlockPtr(Common::ReadStream &f) const override;
	virtual Common::String readMessageString(Common::ReadStream &stream) const override;
	virtual void handleTextOverflow() override;

	void loadItemPictures(Common::ReadStream &stream, byte count);
	virtual void loadRegion(byte region) override;
	void loadWords(Common::ReadStream &stream);
	virtual void readCommands(Common::ReadStream &stream, Commands &commands) override;
